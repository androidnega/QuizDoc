    ...
    <div class="bg-warning-50 border-2 border-warning-300 rounded-lg p-4 flex items-start gap-3">
        ...
        <div class="flex-1">
            ...
            <div class="flex flex-wrap gap-2">
                ...
                {{-- Gemini-only batch generation: 5-question chunks, clear errors (no DeepSeek 402) --}}
                <div id="ai-batch-wrap-{{ $quiz->id }}">
                    <button type="button"
                        ...
                        onclick="startAiBatchGeneration('{{ $quiz->id }}','{{ route('dashboard.quizzes.ai-generate.gemini', $quiz) }}','{{ $generateTopicsStr }}',{{ $quiz->number_of_questions }})">
            ...
            .then(function(data) {
                ...
                if (got === 0) {
                    ...
                    if (zeroRetries >= maxZeroRetries) {
                        showError('Gemini returned 0 questions after ' + maxZeroRetries + ' attempts. Set GEMINI_API_KEY in .env or add key in Settings → AI and enable billing.');
                        return;
                    }
                } else {
                    zeroRetries = 0;
