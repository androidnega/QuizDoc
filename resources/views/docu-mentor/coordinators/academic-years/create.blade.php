...
<div class="w-full min-w-0 max-w-full space-y-6">
    <div class="rounded-lg border border-gray-200 bg-white p-6 max-w-lg">
        <form action="{{ route('dashboard.coordinators.academic-years.store') }}" method="post">
            @csrf
            <div class="space-y-4">
                <div>
                    <label for="year" class="block text-sm font-medium text-gray-700 mb-1">Year (e.g. 2024/2025)</label>
                    <input type="text" name="year" id="year" value="{{ old('year') }}" required class="w-full rounded border-gray-300 text-sm">
                    @error('year')<p class="mt-1 text-sm text-red-600">{{ $message }}</p>@enderror
                </div>
                <div>
                    <label for="submission_deadline" class="block text-sm font-medium text-gray-700 mb-1">Submission deadline (optional, default Sept 30)</label>
                    <input type="date" name="submission_deadline" id="submission_deadline" value="{{ old('submission_deadline') }}" class="w-full rounded border-gray-300 text-sm">
                </div>
                <div class="flex items-center">
                    <input type="checkbox" name="is_active" id="is_active" value="1" {{ old('is_active') ? 'checked' : '' }} class="rounded border-gray-300">
