...
<body class="font-sans text-gray-800 quizsnap-nojs @yield('body_extra_class') @yield('body_class', 'bg-offwhite')" data-skip-guard="{{ $skipGuard ? 'true' : 'false' }}">
    ...
    @endphp
    @if($hasFlash)
    <div id="flash-container" class="fixed top-4 right-4 left-4 sm:left-auto sm:max-w-sm z-50 flex flex-col gap-3 pointer-events-none" role="status" aria-label="Notification">
        @php
