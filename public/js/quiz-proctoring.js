...
(function () {
    ...
    // --- Quiz take page only (this script is not loaded on capture / ready): request browser fullscreen ---
    (function initQuizTakeFullscreen() {
        ...
        function satisfiedFullscreen() {
            return isDocFullscreen() || isFullscreenOrMaximized();
        }
        ...
        function requestDocFullscreen() {
            ...
            try {
                if (docEl.requestFullscreen) {
                    ...
                    p = Promise.reject(new Error('no fullscreen API'));
                }
            } catch (e) {
                p = Promise.reject(e);
            }
        ...
        function removeGate() {
            if (gateEl && gateEl.parentNode) {
                gateEl.parentNode.removeChild(gateEl);
            }
            gateEl = null;
        }
        ...
        function showGate() {
            ...
            gateEl.id = 'quiz-fullscreen-entry-gate';
            gateEl.setAttribute('role', 'dialog');
            gateEl.setAttribute('aria-modal', 'true');
            gateEl.setAttribute('aria-labelledby', 'quiz-fullscreen-entry-title');
            gateEl.className = 'fixed inset-0 z-[58] flex flex-col items-center justify-center bg-gray-900/95 px-4 text-center';
            gateEl.innerHTML = '<div class="max-w-sm w-full rounded-xl bg-white p-5 shadow-xl">' +
                '<p id="quiz-fullscreen-entry-title" class="text-base font-semibold text-gray-900 mb-2">Full screen required</p>' +
                '<p class="text-sm text-gray-600 mb-4">This quiz runs in full screen. Tap or click the button below to continue.</p>' +
                '<button type="button" class="quiz-fullscreen-entry-btn w-full py-3 rounded-xl font-semibold text-white bg-amber-500 hover:bg-amber-600 active:bg-amber-700 touch-manipulation">Enter full screen</button>' +
                '</div>';
            gateEl.querySelector('.quiz-fullscreen-entry-btn').addEventListener('click', function () {
                requestDocFullscreen().then(removeGate).catch(function () {});
            });
            document.body.appendChild(gateEl);
        }

        function tryEnterFromGesture() {
            if (remainingSeconds <= 0) {
                return;
            }
            if (satisfiedFullscreen()) {
                removeGate();
                return;
            }
            requestDocFullscreen().then(removeGate).catch(function () {
        ...
        tryEnterFromGesture();
        setTimeout(function () {
            if (remainingSeconds > 0 && !satisfiedFullscreen()) {
                showGate();
            }
        }, 500);
        ...
        document.addEventListener('mozfullscreenchange', onFullscreenEvent);
    })();
